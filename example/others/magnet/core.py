...
class Magnet2Torrent:
            ...
            for tracker in trackers:
                try:
                    ...
                    if tracker_url.scheme in ["http", "https"]:
                        task = retrieve_peers_http_tracker(
                            task_registry, tracker, infohash
                        )
                        ...
                        logger.warn(
                            "Unknown scheme, {} {}".format(
                                tracker_url.scheme, tracker_url
                            )
                        )
                        continue
    ...
    def get_magnet_info(self, magnet_link: str):
        try:
            ...
            torrent = asyncio.get_event_loop().run_until_complete(
                self.retrieve_torrent(magnet_link)
            )
            return torrent
        except Exception as e:
