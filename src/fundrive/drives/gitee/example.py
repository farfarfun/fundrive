...
def comprehensive_test():
    ...
    add_test("Gitee连接", login_success, "成功连接到仓库" if login_success else "连接失败")
    ...
    print("\n2. 仓库信息测试...")
    quota = drive.get_quota()
    add_test("获取仓库信息", bool(quota), f"仓库大小: {quota.get('size_mb', 0)} MB" if quota else "获取失败")
    ...
    add_test("获取文件信息", info_success, f"大小: {file_info.size if file_info else 0} bytes")
    ...
    download_success = drive.download_file(test_file, download_dir, "downloaded_test.md")
    ...
    while True:
        ...
        try:
            ...
            if choice == "0":
                ...
                download_dir = input("请输入下载目录 (默认: ./downloads): ").strip() or "./downloads"
