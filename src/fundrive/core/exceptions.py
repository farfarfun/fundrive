...
class RateLimitError(FunDriveError):
    ...
    def __init__(self, message: str = "API调用频率超限", retry_after: Optional[int] = None, **kwargs):
...
class FileExistsError(FunDriveError):
    ...
    def __init__(self, message: str = "文件已存在", file_path: Optional[str] = None, **kwargs):
...
class InvalidParameterError(FunDriveError):
    ...
    def __init__(self, message: str = "参数无效", parameter: Optional[str] = None, **kwargs):
...
class OperationNotSupportedError(FunDriveError):
    ...
    def __init__(self, message: str = "操作不支持", operation: Optional[str] = None, **kwargs):
        ...
        def wrapper(*args, **kwargs):
            ...
            for attempt in range(max_retries + 1):
                try:
                    ...
                    logger.warning(f"函数 {func.__name__} 第 {attempt + 1} 次调用失败: {e}，{current_delay:.1f}秒后重试")
...
def handle_api_errors(func: Callable) -> Callable:
    ...
    def wrapper(*args, **kwargs):
        try:
            ...
            elif any(net_err in error_message for net_err in ["connection", "timeout", "network"]):
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                elapsed_time = time.time() - start_time
                logger.error(f"{operation_name} 执行失败，耗时 {elapsed_time:.2f} 秒: {e}")
                raise
